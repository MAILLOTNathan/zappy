...
static void update_client_pos(direction_t dir, amber_trantor_t *trantor,
    amber_world_t *world)
{
    world->_case[trantor->_y][trantor->_x]._players--;
    if (dir == UP)
        trantor->_y = trantor->_y - 1 < 0 ? world->_height - 1 : trantor->_y - 1;
    if (dir == DOWN)
        trantor->_y = trantor->_y + 1 >= world->_height ? 0 : trantor->_y + 1;
    if (dir == RIGHT)
        trantor->_x = trantor->_x + 1 >= world->_width ? 0 : trantor->_x + 1;
    if (dir == LEFT)
        trantor->_x = trantor->_x - 1 < 0 ? world->_width - 1 : trantor->_x - 1;
    world->_case[trantor->_y][trantor->_x]._players++;
}
    ...
    amber_world_t *world, amber_serv_t *serv)
{
    ...
    update_client_pos(trantor->_direction, tmp, world);
